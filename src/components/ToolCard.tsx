...
export default function ToolCard({ 
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    if (!confirm('Are you sure you want to remove this tool from your list?')) {
      return;
    }
    ...
    setIsDeleting(true);
    try {
      // Option 1: Mark as not visible (soft delete)
      const toolRef = doc(db, `users/${user.uid}/tools/${tool.id}`);
  ...
  const cardStyle = {
    ...
    border: isSelected ? '2px solid #38bdf8' : '1px solid rgba(219, 234, 254, 0.1)'
  };
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        right: 0,
        bottom: 0,
        backgroundImage: 'radial-gradient(rgba(219, 234, 254, 0.1) 1px, transparent 1px)',
      ...
      <div style={{position: 'relative', zIndex: 3}}>
        ...
        {/* Description */}
        <p style={descriptionStyle}>{tool.description || 'No description provided'}</p>
        
        {/* Tool metadata */}
        {tool.pricing && (
          <p style={{ fontSize: '0.75rem', color: 'rgba(219, 234, 254, 0.6)', marginBottom: '0.25rem' }}>
            <span style={{ fontWeight: 'bold', color: 'rgba(219, 234, 254, 0.8)' }}>Pricing:</span> {tool.pricing}
          </p>
        )}
        
        {tool.tags && (
          <p style={{ fontSize: '0.75rem', color: 'rgba(219, 234, 254, 0.6)', marginBottom: '0.5rem' }}>
            <span style={{ fontWeight: 'bold', color: 'rgba(219, 234, 254, 0.8)' }}>Tags:</span> {tool.tags}
          </p>
        )}
        ...
        <div style={{ marginTop: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {tool.link && (
            <a 
              ...
              onMouseOver={(e) => {(e.target as HTMLElement).style.color = '#7dd3fc'}}
              onMouseOut={(e) => {(e.target as HTMLElement).style.color = '#38bdf8'}}
              onClick={(e) => e.stopPropagation()}
            >
              {formatLink(tool.link)} 
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style={{width: '14px', height: '14px', marginLeft: '4px'}}>
          ...
          {canEdit && !selectMode && (
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <Link 
                ...
                onMouseOver={(e) => {
                  (e.target as HTMLElement).style.backgroundColor = 'rgba(29, 78, 216, 1)';
                }}
                onMouseOut={(e) => {
                  (e.target as HTMLElement).style.backgroundColor = 'rgba(29, 78, 216, 0.8)';
                }}
                onClick={(e) => e.stopPropagation()}
              >
              ...
              <button
                ...
                style={{
                  fontSize: '0.75rem',
                  padding: '0.25rem 0.75rem',
                  backgroundColor: isDeleting ? 'rgba(185, 28, 28, 0.5)' : 'rgba(185, 28, 28, 0.8)',
                  color: 'white',
                  borderRadius: '0.25rem',
                  display: 'flex',
                  alignItems: 'center',
                ...
                onMouseOver={(e) => {
                  if (!isDeleting) (e.target as HTMLElement).style.backgroundColor = 'rgba(185, 28, 28, 1)';
                }}
                onMouseOut={(e) => {
                  if (!isDeleting) (e.target as HTMLElement).style.backgroundColor = 'rgba(185, 28, 28, 0.8)';
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style={{width: '12px', height: '12px', marginRight: '4px'}}>
      ...
      {selectMode && (
        <div style={{ 
          ...
          <div style={{ 
            ...
            height: '1.5rem', 
            borderRadius: '9999px', 
            backgroundColor: isSelected ? '#38bdf8' : 'rgba(219, 234, 254, 0.2)',
            border: isSelected ? 'none' : '1px solid rgba(219, 234, 254, 0.3)',
            display: 'flex',
            alignItems: 'center',
            ...
            {isSelected && (
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="white" style={{ width: '0.875rem', height: '0.875rem' }}>
